...
class StrokePayAutomation {
  ...
  // 사용자 인터렉션 추적 설정
  async setupUserInteractionTracking() {
    if (!this.page) return
  ...
  async generateAutomationScript(actions) {
  ...
  // 자동화 스크립트 저장
  async saveAutomationScript(script, filename = 'strokepay-automation-script.js') {
    const fs = require('fs')
  ...
  async uploadExcelFile(filePath, inputSelector) {
    if (!this.page) {
      throw new Error('브라우저가 시작되지 않았습니다.')
    }
...
module.exports = { StrokePayAutomation }