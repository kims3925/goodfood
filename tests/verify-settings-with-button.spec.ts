...
test('Verify settings page with Band API button', async ({ page }) => {
  console.log('⚙️ 설정 페이지 및 API 발급 버튼 확인...')
  ...
  // 페이지 요소들 확인
  const pageTitle = await page.locator('h1:has-text("시스템 설정")').isVisible()
  ...
  await page.screenshot({ 
    path: 'screenshots/png/settings-with-api-button.png',
    fullPage: true 
  })
  console.log('📸 최종 스크린샷 저장')
  
  console.log('✅ 설정 페이지 검증 완료!')
})