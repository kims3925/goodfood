...
export default function CollectPage() {
  ...
  // Filter posts based on search and filters, and remove duplicates
  const filteredPosts = (() => {
    // 먼저 기본 필터링을 적용
    const basicFiltered = collectedPosts.filter(post => {
    ...
    // 중복 제거: 제목(hookingTitle 또는 title)이 같은 상품은 1개만 유지
    const uniquePosts = []
    const seenTitles = new Set()

    for (const post of basicFiltered) {
      const titleKey = (post.hookingTitle || post.title || '').toLowerCase().trim()
      if (!seenTitles.has(titleKey) && titleKey) {
  ...
  const handleSourcingConfirm = async (postIds?: string[]) => {
    ...
    try {
      ...
      // 각 게시물의 최종 게시 내용 생성
      const postsWithFinalContent = collectedPosts
        .filter(post => targetPostIds.includes(post.id))
        .map(post => ({
          id: post.id,
      ...
      const response = await fetch('/api/wholesale/posts/confirm', {
        ...
        body: JSON.stringify({
          ...
          finalContents: postsWithFinalContent
        })
      })
      ...
      if (data.success) {
        ...
        if (postIds && showDetailModal) {
          ...
          window.location.href = '/automation/sourcing'
        }
      } else {
  ...
  // 공급가 추출 - 첫 번째 옵션의 가격만 표시 (price.md 기준)
  const getSupplyPrice = (post: CollectedPost) => {
    let basePrice = null

    // 1. 다중 가격 옵션이 있는 경우 첫 번째 옵션의 가격만 사용
    if (post.priceOptions && post.priceOptions.length > 0) {
      const firstOption = post.priceOptions[0]
      basePrice = firstOption.price
      console.log(`📋 다중옵션 중 첫 번째 선택: ${firstOption.option || '옵션1'} - ${basePrice}원 (총 ${post.priceOptions.length}개 옵션)`)
    } else {
      // 2. AI가 추출한 단일 가격 사용
      basePrice = post.extractedPrice || post.originalPrice
      if (basePrice) {
        console.log(`📋 단일 가격 사용: ${basePrice}원`)
      }
    }

    if (!basePrice) {
      console.warn(`⚠️ 가격 정보 없음: 게시물 ${post.id}`)
      return null
    }

    // 3. 밴드별 공급가 책정 로직 적용 (price.md 기준)
    if (post.wholesaleBand.name === '가족도매방' ||
        post.wholesaleBand.name === '요한이네♧소매방' ||
        post.wholesaleBand.name === '초록이네') {
      // 공급가 = 원가의 90%
      const supplyPrice = Math.floor(basePrice * 0.9)
      console.log(`📋 ${post.wholesaleBand.name} 공급가 책정: ${basePrice}원 × 90% = ${supplyPrice}원`)
      return supplyPrice
    }

    // 나머지 밴드들은 공급가 = 원가 그대로
    console.log(`📋 ${post.wholesaleBand.name} 공급가: ${basePrice}원 (원가 그대로)`)
    return basePrice
  }
  ...
  const getSellingPrice = (post: CollectedPost) => {
    ...
    // 실시간 가격정책 적용 계산
    if (!post.wholesaleBand.name) {
      return post.extractedPrice || post.originalPrice // 기본값 반환
    }

    // 다중 옵션이 있는 경우 첫 번째 옵션의 원가로 정책 적용
    let basePrice = null

    // 원가 추출 (price.md 기준 - 판매가는 원가를 기준으로 계산)
    if (post.priceOptions && post.priceOptions.length > 0) {
      basePrice = post.priceOptions[0].price
    } else {
      basePrice = post.extractedPrice || post.originalPrice
    }
    ...
    const parsedOriginalPrice = parsePrice(basePrice)
    const shippingFee = typeof post.shippingFee === 'number' ? post.shippingFee : 0

    return applyPricingPolicy(parsedOriginalPrice, shippingFee, post.wholesaleBand.name)
  }
  ...
  // 개별 가격 옵션에 정책 적용하는 헬퍼 함수 (price.md 기준) - 하위 호환성 유지
  const applyPolicyToPrice = (price: number, bandName: string) => {
    if (!price || price <= 0) {
      return price
    }

    const parsedPrice = parsePrice(price)
    return applyPricingPolicy(parsedPrice, 0, bandName)
  }
  ...
  // 판매가 옆 배송비 정보 (괄호 형태)
  const getShippingInfo = (post: CollectedPost) => {
    // 1. 배송비 정책(shippingPolicy) 우선 확인
    if (post.shippingPolicy) {
      const lowerPolicy = post.shippingPolicy.toLowerCase()
      if (lowerPolicy.includes('무료') || lowerPolicy.includes('포함')) {
        return '(무료배송)'
      }
      if (lowerPolicy.includes('별도')) {
        return '(배송비 별도)'
      }
      return `(${post.shippingPolicy})`
    }

    // 2. 배송비 수치(shippingFee) 확인
    if (post.shippingFee !== null && post.shippingFee !== undefined) {
      if (post.shippingFee === 0) {
        return '(무료배송)'
      }
      if (typeof post.shippingFee === 'number') {
        return `(배송비 ${formatPrice(post.shippingFee)})`
      }
      if (typeof post.shippingFee === 'string' && post.shippingFee) {
        const lowerFee = (post.shippingFee as string).toLowerCase()
        if (lowerFee.includes('무료') || lowerFee.includes('포함')) {
          return '(무료배송)'
        }
        if (lowerFee.includes('별도')) {
          return '(배송비 별도)'
        }
        // "3000원", "3,000원" 같은 형태를 숫자로 파싱
        const numericFee = parsePrice(post.shippingFee)
        if (numericFee > 0) {
          return `(배송비 ${numericFee.toLocaleString()}원)`
        }
        return `(${post.shippingFee})`
      }
    }

    // 3. 게시물 내용에서 배송비 정보 추출 시도
    const content = post.content || ''
    const lowerContent = content.toLowerCase()

    // "무료배송" 키워드 검색
    if (lowerContent.includes('무료배송') || lowerContent.includes('무배')) {
      return '(무료배송)'
    }
    ...
    // 나은 상품 공급방, S D 푸드, 폐쇄몰VIP도매: 배송비 별도 정책
    if (bandName === '나은 상품 공급방' || bandName === 'S D 푸드' || bandName === '폐쇄몰VIP도매') {
      return '(배송비 별도)'
    }

    // 가족도매방, 요한이네♧소매방, 초록이네: 일반적으로 배송비 포함
    if (bandName === '가족도매방' || bandName === '요한이네♧소매방' || bandName === '초록이네') {
      return '(무료배송)'
    }
  ...
  // 최종 게시 내용 생성 함수 (소싱확정에서도 사용)
  const generateFinalPostingContent = (post: CollectedPost) => {
    const finalContent = []

    // 후킹 내용이 있으면 사용, 없으면 상세 내용 사용
    const mainContent = post.hookingContent || post.detailedContent || post.content
    if (mainContent) {
    ...
    if (post.priceOptions && post.priceOptions.length >= 2) {
      finalContent.push('')
      finalContent.push('📋 가격 옵션:')
      post.priceOptions.forEach((option: any, index: number) => {
        // 각 옵션에 가격정책 적용 (밴드명 기준)
        const finalPrice = applyPolicyToPrice(option.price, post.wholesaleBand.name)

        const optionName = option.option || `옵션${index + 1}`
        const shippingInfo = getShippingInfo(post)
        finalContent.push(`${index + 1}. ${optionName}: ${finalPrice.toLocaleString()}원 ${shippingInfo}`)
      })
      priceAdded = true
    } else {
      // 옵션이 1개 이하인 경우: 정책 적용된 쇼핑몰 판매가 사용
      const basePrice = post.extractedPrice || post.originalPrice
      if (basePrice) {
        const finalPrice = applyPolicyToPrice(basePrice, post.wholesaleBand.name)
        finalContent.push('')
      ...
      if (post.priceOptions && post.priceOptions.length === 1) {
        const option = post.priceOptions[0]
        // 옵션 가격에도 정책 적용 (밴드명 기준)
        const finalPrice = applyPolicyToPrice(option.price, post.wholesaleBand.name)

        finalContent.push('')
        const shippingInfo = getShippingInfo(post)
        finalContent.push(`📋 ${option.option}: ${finalPrice.toLocaleString()}원 ${shippingInfo}`)
        priceAdded = true
      }
    ...
    if (post.shippingPolicy) {
      finalContent.push('')
      finalContent.push(`🚚 배송: ${post.shippingPolicy}`)
    } else if (post.shippingFee !== null && post.shippingFee !== undefined) {
      finalContent.push('')
      if (post.shippingFee === 0) {
        finalContent.push('🚚 배송비: 무료배송')
      } else {
        finalContent.push(`🚚 배송비: ${post.shippingFee.toLocaleString()}원`)
      }
    }
    ...
    if (post.hasDeadline && post.deadlineInfo) {
      ...
      finalContent.push(post.deadlineInfo)
    }
  ...
  // 가격이 포함된 이미지 감지 및 필터링
  const filterPriceImages = (images: string[]) => {
    // 가격 표시가 포함될 가능성이 높은 이미지 URL 패턴 및 파일명
    const priceIndicatorPatterns = [
      /price/i,
      /money/i,
      /won/i,
      /\d+원/,
    ...
    return images.filter((imageUrl, index) => {
      // 첫 번째 이미지는 대표 이미지로 보통 유지
      if (index === 0) return true

      // URL에 가격 관련 키워드가 있는지 확인
      const hasKeyword = priceIndicatorPatterns.some(pattern =>
        pattern.test(imageUrl)
      )

      // 가격 관련 키워드가 있으면 제외
      if (hasKeyword) {
        console.log(`🖼️ 가격 관련 이미지 제외: ${imageUrl.substring(0, 50)}...`)
        return false
      }
  ...
  // 강화된 중복 상품 체크 함수
  const checkForDuplicates = (posts: CollectedPost[]) => {
    const titleMap = new Map()
    const imageMap = new Map()
    const contentMap = new Map()
    const duplicateGroups: CollectedPost[][] = []

    posts.forEach(post => {
      // 1. 제목 기반 중복 체크 (더 엄격하게)
      ...
      if (cleanTitle && cleanTitle.length > 5) {
        if (titleMap.has(cleanTitle)) {
          titleMap.get(cleanTitle).push(post)
        } else {
          titleMap.set(cleanTitle, [post])
        }
      }
      ...
      if (firstImage) {
        if (imageMap.has(firstImage)) {
          imageMap.get(firstImage).push(post)
        } else {
          imageMap.set(firstImage, [post])
        }
      }

      // 3. 콘텐츠 기반 중복 체크 (새로 추가)
      ...
      if (contentKey && contentKey.length > 10) {
        if (contentMap.has(contentKey)) {
          contentMap.get(contentKey).push(post)
        } else {
    ...
    // 중복 그룹 수집 (각 기준별로)
    const allMaps = [titleMap, imageMap, contentMap]
    const processedIds = new Set()

    allMaps.forEach((map, mapIndex) => {
      map.forEach(group => {
        if (group.length > 1) {
          // 이미 다른 그룹에 포함된 게시물이 있는지 확인
          const hasProcessedPost = group.some((post: any) => processedIds.has(post.id))

          if (!hasProcessedPost) {
            duplicateGroups.push(group)
            group.forEach((post: any) => processedIds.add(post.id))

            // 중복 유형 로깅
            const duplicateType = mapIndex === 0 ? '제목' : mapIndex === 1 ? '이미지' : '내용'
            console.log(`🔍 ${duplicateType} 기반 중복 그룹 발견: ${group.length}개 게시물`)
          }
        }
      })
    })
  ...
  // 중복 게시물 ID 목록 반환 (모든 중복)
  const getDuplicatePostIds = (posts: CollectedPost[]) => {
    const duplicateGroups = checkForDuplicates(posts)
    const duplicateIds = new Set<string>()

    duplicateGroups.forEach(group => {
      group.forEach(post => duplicateIds.add(post.id))
    })

    return duplicateIds
  }

  // 중복 게시물 중 최신 것(뒤에 있는 것) 1개씩만 선택하여 삭제 대상 반환
  const getLatestDuplicatePostIds = (posts: CollectedPost[]) => {
    const duplicateGroups = checkForDuplicates(posts)
    const latestDuplicateIds: string[] = []

    duplicateGroups.forEach(group => {
      if (group.length > 1) {
        // 날짜순으로 정렬하여 가장 최신(뒤에 있는) 게시물 1개만 선택
        const sortedGroup = group.sort((a, b) => {
          const dateA = new Date(a.bandCreatedAt).getTime()
          const dateB = new Date(b.bandCreatedAt).getTime()
          return dateB - dateA // 내림차순 정렬 (최신이 앞으로)
        })

        // 가장 최신 게시물 1개만 삭제 대상으로 선택 (첫 번째는 유지, 나머지 삭제)
        if (sortedGroup.length > 1) {
          latestDuplicateIds.push(sortedGroup[0].id) // 가장 최신 것을 삭제
        }

        console.log(`🗂️ 중복 그룹: ${group.length}개 → 최신 1개 삭제 선택: ${sortedGroup[0].title?.slice(0, 20)}...`)
      }
    })

    return latestDuplicateIds
  }

  // 게시물이 중복인지 확인
  const isDuplicatePost = (postId: string) => {
    const duplicateIds = getDuplicatePostIds(filteredPosts)
    return duplicateIds.has(postId)
  }

  // 게시물이 삭제 대상 중복인지 확인 (최신 중복)
  const isLatestDuplicatePost = (postId: string) => {
    const latestDuplicateIds = getLatestDuplicatePostIds(filteredPosts)
    return latestDuplicateIds.includes(postId)
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">게시물수집</h1>
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    const latestDuplicateIds = getLatestDuplicatePostIds(filteredPosts)
                    const totalDuplicateCount = Array.from(getDuplicatePostIds(filteredPosts)).length

                    if (latestDuplicateIds.length > 0) {
                      if (confirm(`${totalDuplicateCount}개의 중복 게시물 중 최신 ${latestDuplicateIds.length}개를 삭제하시겠습니까?\n(중복 그룹별로 최신 게시물 1개씩만 삭제하여 각 상품당 1개의 게시물만 남깁니다)`)) {
                        setSelectedPosts(latestDuplicateIds)
                        handleBulkDelete()
                      }
                    } else {
                      alert('중복 게시물이 없습니다.')
                ...
                >
                  ...
                  중복 제거 ({getLatestDuplicatePostIds(filteredPosts).length}/{Array.from(getDuplicatePostIds(filteredPosts)).length})
                </button>
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {filteredPosts.map((post) => (
                  ...
                  >
                    ...
                    {isDuplicatePost(post.id) && !isLatestDuplicatePost(post.id) && (
                      <div className="absolute -top-1 -right-1 bg-orange-500 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1">
                        <XCircle className="h-3 w-3" />
                        중복
                      </div>
                    )}
                            ...
                            {getSellingPrice(post) ? (
                              <>
                                ...
                                <span className="text-sm font-semibold text-blue-600">
                                  판매가: {formatPrice(getSellingPrice(post) || 0)} {getShippingInfo(post)}
                                </span>
                              </>
                            ) : (
                    ...
                    {post.images && post.images.length > 0 && (
                      <div className="mb-3">
                        <div className="grid grid-cols-3 gap-2">
                          {filterPriceImages(post.images).slice(0, 3).map((imageUrl, index) => (
                            <div key={index} className="relative">
                              ...
                              />
                              {filterPriceImages(post.images).length > 3 && index === 2 && (
                                <div className="absolute inset-0 bg-black bg-opacity-50 rounded flex items-center justify-center">
                                  <span className="text-white text-xs font-medium">
                                    +{filterPriceImages(post.images).length - 3}
                                  </span>
                                </div>
                              )}
                            </div>
                              ...
                              {getSellingPrice(post) ? (
                                <>
                                  ...
                                  <span className="text-lg font-semibold text-blue-600">
                                    {formatPrice(getSellingPrice(post) || 0)} {getShippingInfo(post)}
                                  </span>
                                </>
                              ) : (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '40px' }}>
                          선택
                        </th>
                        <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '560px' }}>
                          제목
                        </th>
                        <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '60px' }}>
                          분류
                        </th>
                        <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '100px' }}>
                          <div>공급가</div>
                          <div className="text-xs text-gray-400 normal-case">(첫째 옵션)</div>
                        </th>
                        <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '100px' }}>
                          <div>판매가</div>
                          <div className="text-xs text-gray-400 normal-case">(정책적용)</div>
                        </th>
                        <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '100px' }}>
                          배송비
                        </th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredPosts.map((post) => (
                        <tr key={post.id} className={`hover:bg-gray-50 ${
                          ...
                          <td className="px-2 py-4 whitespace-nowrap" style={{ width: '40px' }}>
                            <input
                              type="checkbox"
                              checked={selectedPosts.includes(post.id)}
                              onChange={() => handlePostToggle(post.id)}
                          ...
                          <td className="px-2 py-4" style={{ width: '560px' }}>
                            <div className="text-sm font-medium text-gray-900">
                              {truncateText(post.hookingTitle || post.title, 100)}
                            </div>
                            <div className="text-xs text-gray-500">
                              {truncateText(post.hookingContent || post.detailedContent || post.content, 120)}
                            </div>
                          </td>
                          ...
                          <td className="px-2 py-4 whitespace-nowrap" style={{ width: '100px' }}>
                            <div className="text-sm font-semibold text-green-600">
                              <>
                                ...
                                {post.priceOptions && post.priceOptions.length > 1 && (
                                  <div className="text-xs text-gray-500 mt-1">
                                    +{post.priceOptions.length - 1}옵션 더
                                  </div>
                                )}
                                {post.salesUnit && (
                          ...
                          <td className="px-2 py-4 whitespace-nowrap" style={{ width: '100px' }}>
                            <div className="space-y-1">
                              <span className={`inline-flex px-1 py-1 text-xs font-semibold rounded ${
                                ...
                                {post.status === 'PROCESSED' ? '완료' : post.status === 'SELECTED' ? '선택' : '대기'}
                              </span>
                              <div className="flex flex-wrap gap-1">
                                {post.aiAnalyzed && (
                        ...
                        <>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                              {/* 다중 가격 옵션 표시 - 정책 적용된 가격 */}
                              {selectedPostForDetail.priceOptions && selectedPostForDetail.priceOptions.length > 0 && (
                                <div className="bg-white p-3 rounded border">
                                  <h5 className="text-sm font-semibold text-green-800 mb-3">💰 추출된 옵션 (정책 적용)</h5>
                                  <div className="space-y-2">
                                    {selectedPostForDetail.priceOptions.map((option, index) => {
                                      // 각 옵션에 가격정책 적용 (price.md 기준)
                                      const bandName = selectedPostForDetail.wholesaleBand.name
                                      const finalPrice = applyPolicyToPrice(option.price, bandName)
                                        ...
                                        <div key={index} className="flex justify-between items-center py-2 px-3 bg-green-50 rounded-lg">
                                          ...
                                          <div className="flex flex-col items-end">
                                            <div className="flex flex-col items-end space-y-1">
                                              <span className="text-xs text-gray-500">
                                                원가: {formatPrice(option.price)}
                                              </span>
                                              <span className="text-base font-bold text-blue-600">
                                                쇼핑몰 판매가: {formatPrice(finalPrice)}
                                              </span>
                                              <span className="text-sm text-green-600">
                                                공급가: {formatPrice(getSupplyPrice(selectedPostForDetail) || 0)}
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                              ...
                              {selectedPostForDetail.extractedPrice && (!selectedPostForDetail.priceOptions || selectedPostForDetail.priceOptions.length === 0) && (
                                <div className="bg-white p-3 rounded border">
                                  <div className="flex justify-between items-center">
                                    <span className="text-sm text-gray-600">가격 정보</span>
                                    <div className="flex flex-col items-end space-y-1">
                                      <span className="text-xs text-gray-500">
                                        원가: {formatPrice(selectedPostForDetail.extractedPrice)}
                                      </span>
                                      <span className="text-base font-bold text-blue-600">
                                        쇼핑몰 판매가: {formatPrice(applyPolicyToPrice(selectedPostForDetail.extractedPrice, selectedPostForDetail.wholesaleBand.name))}
                        ...
                        <div>
                          ...
                          <div className="bg-gray-50 p-4 rounded border">
                            <div className="whitespace-pre-wrap text-gray-900 text-base leading-relaxed">
                              {generateFinalPostingContent(selectedPostForDetail)}
                            </div>
                          </div>
                        </div>
                ...
                {selectedPostForDetail.images.length > 0 && (
                  <div className="border-t pt-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                      ...
                      이미지 갤러리 ({filterPriceImages(safeParseImages(selectedPostForDetail.images)).length}개)
                    </h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                      {filterPriceImages(safeParseImages(selectedPostForDetail.images)).map((imageUrl: string, index: number) => (
                        <div key={index} className="aspect-square group relative">
                          <img
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              const allImages = filterPriceImages(safeParseImages(selectedPostForDetail.images))
                ...
                <div className="mt-6 bg-gray-50 p-4 rounded-lg border-t">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{filterPriceImages(safeParseImages(selectedPostForDetail.images)).length}개 (가격이미지 제외)</p>
                    </div>
                  </div>
                </div>
              </div>
