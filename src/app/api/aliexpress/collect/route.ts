...
import { analyzeProductContent, analyzeProductContentWithPolicy } from '@/domain/wholesale/services/ai-analysis.service'
import { getAliExpressAPI } from '@/domain/aliexpress/services/api-client.service'
import { getExchangeRate, parsePricingPolicy, calculateAliExpressPrice } from '@/lib/helpers/currency'
  ...
  try {
      ...
      if (cleanTitle && cleanTitle.length > 10) {
        ...
        if (keywords.length > 0) {
          ...
          if (similarProducts.length > 0) {
            console.log(`🔄 키워드 기반 유사 상품 존재: ${similarProducts[0].title?.slice(0, 30)}...`)
            continue
          }
        }
    ...
    for (const product of uniqueProducts) {
      try {
        ...
        const aiAnalysis = sourcing.pricingPolicy
          ? await analyzeProductContentWithPolicy(product.productTitle, contentForAI, [], sourcing.pricingPolicy)
          : await analyzeProductContent(product.productTitle, contentForAI, [])
        ...
        const savedProduct = await prisma.aliExpressProduct.upsert({
          ...
          create: {
            ...
            sourcingId: sourcing.id,
            productId: product.productId,
            title: sanitizeContent(product.productTitle),
            productUrl: product.productUrl,
            description: sanitizeContent(product.description || product.productTitle),
            originalPrice: priceCalculation.originalPriceKRW,
            discount: product.discount || 0,
            currency: product.currency || 'USD',
            minOrderQty: product.minOrderQty || 1,
            shippingCost: Math.round(product.shippingPrice || 0),
            shippingDays: product.shippingDays || null,
            supplier: product.supplier || null,
            rating: product.rating || null,
            totalOrders: product.totalOrders || null,
            status: 'PENDING',
            // AI 분석 결과
            aiAnalyzed: true,
            ...
            finalShippingFee: Math.round(product.shippingPrice || 0),
            policyApplied: true,
            priceOptions: JSON.stringify(aiAnalysis?.priceOptions || []),
        ...
        // 이미지 저장 (별도 테이블)
        if (product.productImage) {
          await prisma.aliExpressProductImage.create({
            data: {
