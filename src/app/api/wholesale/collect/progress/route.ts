...
import { analyzeProductContent, analyzeProductContentWithPolicy, parallelBatchAnalyzeProducts } from '@/domain/wholesale/services/ai-analysis.service'
import { validateBandToken, ensureValidToken } from '@/domain/band'
...
export async function GET(request: NextRequest) {
  ...
  let bandId: number, dateRange
  try {
    const parsedData = JSON.parse(dataParam)
    bandId = parseInt(parsedData.bandId, 10)
...
export async function POST(request: NextRequest) {
  ...
  return handleCollectionProgress(parseInt(bandId, 10), dateRange)
}

async function handleCollectionProgress(bandId: number, dateRange?: any) {
  try {
    ...
    // 데이터베이스에서 밴드 API 토큰 로드
    let accessToken: string
    try {
      accessToken = await getBandAccessToken(actualUser.id)
    } catch (error: any) {
      return NextResponse.json({
        success: false,
        error: error.message || 'Band API 설정을 불러올 수 없습니다.'
      }, { status: 400 })
    }
        ...
        try {
          ...
          for (let i = 0; i < newPostsData.length; i++) {
            ...
            try {
              ...
              // Create PostImage records separately
              if (item.images && Array.isArray(item.images) && item.images.length > 0) {
                await prisma.postImage.createMany({
                  data: item.images.map((imageUrl: string, index: number) => ({
                    postId: savedPost.id,
                    url: imageUrl,
                    sortOrder: index
                  }))
          ...
          const commentsCollectedCount = 0 // Comments are no longer collected
