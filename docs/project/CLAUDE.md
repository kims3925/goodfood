...
> **최종 업데이트**: 2026-04-09 (라이브 서버 `/sourcing/settings/prompt?tab=policy` 기준)
>
> 이전 문서(요한이네♧소매방 중심)는 현재 운영 정책과 달라 전면 교체됨.
> **ProductManagerAgent SKILL 1** 검증 로직도 이 정책 기준으로 동기화됨.

현재 시스템에는 **6개의 도매밴드**가 등록되어 있으며, 각각 고유한 가격정책을 가지고 있습니다.

### 🔑 용어 정의

1. **도매가(wholesalePrice)**: 도매밴드에서 제공하는 공급가 (고객 노출 금지)
2. **배송비(shippingFee)**: 무료=0, 배송비포함=합배송 차감금액, 배송비별도=실제 배송비
3. **기준가**: 마진 구간 결정 기준 — 무료/포함 시 = 도매가 / 배송비별도 시 = 도매가+배송비
4. **판매가(price)**: 기준가 + 마진 (고객에게 노출되는 가격)
...
### 📋 정책 개요
- **유형**: 마진 없음 — 도매가 = 판매가
- **특징**: 가격 그대로 적용, 배송비 별도 시 도매가+배송비가 최종 판매가
- **DB 정책명**: 킹도매방

### 💰 가격 적용 로직

#### 기준
- 무료배송 / 배송비 포함: `판매가 = 도매가` (마진 0)
- 배송비 별도: `판매가 = 도매가 + 배송비` (마진 0)

#### shippingFee 저장 규칙
- 무료배송: `shippingFee = 0 또는 null`
- 배송비 포함: `shippingFee = 합배송 차감 금액`
- 배송비 별도: `shippingFee = 실제 배송비`

#### 예시
```
도매가: 25,000원, 배송비: 무료
→ wholesalePrice=25000, price=25000, shippingFee=0

도매가: 25,000원, 배송비: 3,000원 별도
→ wholesalePrice=25000, price=28000, shippingFee=3000
```
...
### 📋 정책 개요
- **유형**: 구간별 마진 적용 (40,000원 이상 소싱 제외)
- **특징**: 공급가 표시 상품에만 마진 적용, 판매가 표시 상품은 그대로
- **DB 정책명**: 가족도매방기본마진정책
...
```
도매가: 25,000원, 배송비: 무료
→ 기준가 25,000 → 구간 19,901~29,900 → 마진 +5,000
→ wholesalePrice=25000, price=30000, shippingFee=0

도매가: 35,000원, 배송비: 3,000원 별도
→ 기준가 38,000 → 구간 29,901~39,900 → 마진 +6,000
→ wholesalePrice=35000, price=44000, shippingFee=3000
```
...
### 📋 정책 개요
- **유형**: 최소 마진 구간 적용 (40,000원 이상 소싱 제외)
- **특징**: 마진이 매우 낮음 (19,900원 이하는 마진 0)
- **DB 정책명**: 초록이네
...
|------------|------|
| 1원 ~ 19,900원 | +0원 (마진 없음) |
| 19,901원 ~ 29,900원 | +1,000원 |
| 29,901원 ~ 39,900원 | +2,000원 |
| 40,000원 이상 | **소싱 제외** |
...
```
도매가: 15,000원, 배송비: 무료
→ 기준가 15,000 → 구간 1~19,900 → 마진 +0
→ wholesalePrice=15000, price=15000, shippingFee=0

도매가: 25,000원, 배송비: 무료
→ 기준가 25,000 → 구간 19,901~29,900 → 마진 +1,000
→ wholesalePrice=25000, price=26000, shippingFee=0
```
...
### 📋 정책 개요
- **유형**: 구간별 마진 적용, 고가 동적 구간
- **특징**: 폐쇄몰VIP도매, SD푸드와 동일한 마진 구조
- **DB 정책명**: 나은
...
| 19,901원 ~ 29,900원 | +5,000원 |
| 29,901원 ~ 39,900원 | +6,000원 |
| 39,901원 ~ 49,900원 | +7,000원 |
| 49,901원 ~ 59,900원 | +8,000원 |
| 59,901원 ~ 69,900원 | +9,000원 |
| 69,901원 ~ 79,900원 | +10,000원 |
| 79,901원 ~ 89,900원 | +11,000원 |
| 89,901원 ~ 99,900원 | +12,000원 |
...
도매가: 95,000원, 배송비: 5,000원 별도
→ 기준가 100,000 → 구간 99,901+ → 마진 +13,000
→ wholesalePrice=95000, price=113000, shippingFee=5000
```
...
### 📋 정책 개요
- **유형**: 나은 상품 공급방과 동일한 마진 구조
- **특징**: 주로 댓글에 배송비 정보 포함
- **DB 정책명**: sd푸드

### 💰 가격 적용 로직
**나은 상품 공급방(4️⃣)과 완전히 동일한 마진 구간표 적용**
...
#### 예시
```
도매가: 18,000원, 배송비: 3,000원 별도 (댓글에서 확인)
→ 기준가 21,000 → 구간 19,901~29,900 → 마진 +5,000
→ wholesalePrice=18000, price=26000, shippingFee=3000
```
...
```
도매가: 35,000원, 배송비: 포함
→ 기준가 35,000 → 구간 29,901~39,900 → 마진 +6,000
→ wholesalePrice=35000, price=41000, shippingFee=4000 (합배송 차감용)

도매가: 48,000원, 배송비: 4,000원 별도
→ 기준가 52,000 → 구간 49,901~59,900 → 마진 +8,000
→ wholesalePrice=48000, price=60000, shippingFee=4000
```
...
### ProductManagerAgent SKILL 1 검증 로직
- **위치**: `sourcing-app/src/modules/agents/implementations/ProductManagerAgent.ts`
- **방식**: `product.channelId`(도매채널)로 DB에서 `PricingPolicy.content` 조회 후 파싱
- **파서**: `parsePricingPolicyRules()` — 마크다운 테이블 포맷 + 킹도매방 "판매가 그대로" 처리
- **배송비 처리**: Case A(무료/포함: 기준가=도매가) / Case B(별도: 기준가=도매가+배송비) 동시 검증
- **동적 구간**: 99,901원 이상 → 기본 13,000원 + (기준가-99,901) ÷ 10,000 × 1,000원
- **허용 오차**: ±100원 (반올림 허용)

### 도매밴드별 마진 구조 요약

| 도매밴드 | 정책명 | 마진 특징 | 제외 조건 |
|---------|--------|----------|----------|
| 킹도매방 | 킹도매방 | **마진 0** (도매가=판매가) | 없음 |
| 가족도매방 | 가족도매방기본마진정책 | +4,000~+6,000원 | ≥40,000원 제외 |
| 초록이네 | 초록이네 | +0~+2,000원 (최소) | ≥40,000원 제외 |
| 나은 상품 공급방 | 나은 | +4,000~+13,000원+ | 없음 |
| S D 푸드 | sd푸드 | 나은과 동일 | 없음 |
| 폐쇄몰VIP도매 | vip | 나은과 동일 | 없음 |
