...
export default function CollectPage() {
  ...
  const getSupplyPrice = (post: CollectedPost) => {
    ...
    if (post.priceOptions && post.priceOptions.length > 0) {
      ...
      console.log(`📋 다중옵션 중 첫 번째 선택: ${firstOption.name || '옵션1'} - ${basePrice}원 (총 ${post.priceOptions.length}개 옵션)`)
    } else {
      // 2. AI가 추출한 단일 가격 사용
  ...
  const getShippingInfo = (post: CollectedPost) => {
    ...
    if (post.shippingFee !== null && post.shippingFee !== undefined) {
      ...
      if (typeof post.shippingFee === 'string') {
        const lowerFee = post.shippingFee.toLowerCase()
    ...
    allMaps.forEach((map, mapIndex) => {
      map.forEach(group => {
        if (group.length > 1) {
          ...
          const hasProcessedPost = group.some(post => processedIds.has(post.id))

          if (!hasProcessedPost) {
            duplicateGroups.push(group)
            group.forEach(post => processedIds.add(post.id))
