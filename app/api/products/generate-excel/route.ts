...
export async function POST(request: NextRequest) {
  try {
    ...
    products.forEach((product, index) => {
      ...
      // 카테고리 정보
      const detailedCategory = CATEGORY_MAPPING[product.productCategory as keyof typeof CATEGORY_MAPPING] || '식품>밀키트>세트요리'
      const displayCategory = DISPLAY_CATEGORY_MAPPING[product.productCategory as keyof typeof DISPLAY_CATEGORY_MAPPING] || '기타'
      ...
      if (options.length <= 1) {
        ...
        })).sort((a, b) => a.salePrice - b.salePrice) // 판매가 기준으로 오름차순 정렬
        
        // 정렬된 옵션에서 데이터 추출
        const sortedOptionNames = sortedOptions.map(item => item.option)
        const sortedOriginalPrices = sortedOptions.map(item => item.originalPrice)
        const sortedSalePrices = sortedOptions.map(item => item.salePrice)
        
        optionValues = sortedOptionNames.join(',')
        
        // 첫 번째 옵션(최저가)은 공급가와 판매가격을 "0"으로 설정
        const adjustedSupplyPrices = [0, ...sortedOriginalPrices.slice(1).map(price => 
          price - sortedOriginalPrices[0] // 첫 번째 옵션 대비 추가 금액만
        )]
        
        const adjustedOptionAmounts = [0, ...sortedSalePrices.slice(1).map(price => 
