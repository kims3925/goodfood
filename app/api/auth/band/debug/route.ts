...
export async function GET(request: NextRequest) {
  try {
    ...
    const config = {
      ...
      BAND_CLIENT_SECRET: process.env.BAND_CLIENT_SECRET ? '설정됨 (길이: ' + process.env.BAND_CLIENT_SECRET.length + ')' : '❌ 미설정',
      BAND_REDIRECT_URI: process.env.BAND_REDIRECT_URI,
      BAND_ACCESS_TOKEN: process.env.BAND_ACCESS_TOKEN ? '설정됨 (길이: ' + process.env.BAND_ACCESS_TOKEN.length + ')' : '❌ 미설정'
    }
    ...
    if (!process.env.BAND_CLIENT_ID) {
      validationErrors.push('BAND_CLIENT_ID가 설정되지 않음')
    }
    
    if (!process.env.BAND_CLIENT_SECRET) {
      validationErrors.push('BAND_CLIENT_SECRET가 설정되지 않음')
    }
    
    if (!process.env.BAND_REDIRECT_URI) {
      validationErrors.push('BAND_REDIRECT_URI가 설정되지 않음')
    }
    ...
    if (validationErrors.length === 0) {
      try {
        ...
        authUrl.searchParams.append('client_id', process.env.BAND_CLIENT_ID!)
        authUrl.searchParams.append('redirect_uri', process.env.BAND_REDIRECT_URI!)
    ...
    if (validationErrors.length > 0) {
      diagnostics.push({
        ...
        solution: '.env.local 파일에서 누락된 환경변수를 설정하세요.'
      })
    }
