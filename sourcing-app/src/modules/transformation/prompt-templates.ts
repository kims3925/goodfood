...
### ⭐ "N종선택" / "N종 세트" 패턴 (다중 선택) — 필수 처리

도매처 게시글에 "**2종선택**", "**3종선택**", "**N종 세트**", "**N종 묶음**", "**N종 혼합**"
같이 **고객이 여러 종류 중 N개를 골라 묶음으로 사는** 패턴이 있으면:

**variants 처리 방법**: 종류 자체는 variant 로 만들지 말고, **묶음 수량만** variant 로 등록.
종류 선택은 고객이 주문 후 댓글로 알려주는 방식 (시스템이 자유 다중 선택 옵션을 지원하지 않음).

예시 입력:
\`\`\`
한봉 650g (6,000원)
- 흰쌀강정 / 찐쌀강정 / 보리강정 / 오란다 / 강냉이혼합 / 5종혼합
...
\`\`\`json
[
  { "optionSummary": "650g 1봉 (단품)", "options": {"구성": "1봉"}, "wholesalePrice": 6000, "price": 6000, "bundleUnit": 1 },
  { "optionSummary": "650g 2종 세트",     "options": {"구성": "2종 세트"}, "wholesalePrice": 11000, "price": 11000, "bundleUnit": 1 },
  { "optionSummary": "650g 3종 세트",     "options": {"구성": "3종 세트"}, "wholesalePrice": 16000, "price": 16000, "bundleUnit": 1 }
]
\`\`\`

**description 에 필수 안내 추가**: 다중 선택 패턴이 감지되면 description 마지막에
"📝 비고/요청사항" 섹션을 두고 다음 한 줄을 **반드시 포함**:

\`\`\`
📝 비고/요청사항
• 종류 선택: 주문 후 댓글로 원하시는 종류를 알려주세요.
\`\`\`

가능하면 종류 후보도 한 줄로 요약 (예: "650g: 흰쌀/찐쌀/보리/오란다/강냉이/5종혼합 中 N가지").

이 안내가 없으면 도매처 발주 시 어떤 종류를 보낼지 모름 — **누락 금지**.
