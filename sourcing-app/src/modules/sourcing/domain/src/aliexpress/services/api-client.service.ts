...
export class AliExpressAPI {
  ...
  private parseProduct(raw: AliExpressAPIRawProduct): AliExpressProduct {
    return {
      ...
      originalPrice: parseFloat(String(raw.original_price || raw.target_original_price || 0)),
      salePrice: parseFloat(String(raw.target_sale_price || raw.sale_price || 0)),
      discount: parseInt(String(raw.discount || 0)),
      currency: raw.target_sale_price_currency || raw.original_price_currency || 'USD',
      rating: raw.evaluate_rate ? parseFloat(String(raw.evaluate_rate)) : undefined,
      totalOrders: raw.volume ? parseInt(String(raw.volume)) : undefined,
      shippingPrice: raw.estimated_price_ship ? parseFloat(String(raw.estimated_price_ship)) : undefined,
