...
export async function GET(request: NextRequest) {
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.termsPolicy.findMany({
        where,
        orderBy: [{ main: 'desc' }, { sub: 'desc' }],
        skip: (page - 1) * limit,
        take: limit,
      }),
      prisma.termsPolicy.count({ where }),
    ])
...
export async function POST(request: NextRequest) {
  try {
    ...
    const latestPolicy = await prisma.termsPolicy.findFirst({
      orderBy: [{ main: 'desc' }, { sub: 'desc' }],
    })
    ...
    const newPolicy = await prisma.termsPolicy.create({
      data: {
        name,
        content,
        main,
