...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('[ë¡ê·¸ì¸] ìì²­ ìì:', formData.email)
      ...
      console.log('[ë¡ê·¸ì¸] ìëµ ìí:', response.status)

      const data = await response.json()
      console.log('[ë¡ê·¸ì¸] ìëµ ë°ì´í°:', data)

      if (data.success) {
        console.log('[ë¡ê·¸ì¸] ì±ê³µ! ì­í :', data.user?.role)

        // ADMIN, MANAGERê° ìëë©´ ì ê·¼ ê±°ë¶ íì´ì§ë¡
        if (data.user?.role !== 'ADMIN' && data.user?.role !== 'MANAGER') {
          window.location.href = '/unauthorized'
          return
        }

        // redirect íë¼ë¯¸í°ê° ìì¼ë©´ í´ë¹ ê²½ë¡ë¡, ìì¼ë© ë§¤ëì  ëìë³´ëë¡
        ...
        console.log('[ë¡ê·¸ì¸] ì¤í¨:', data.error)
        setError(data.error || 'ë¡ê·¸ì¸ì ì¤í¨íìµëë¤.')
      }
    } catch (error) {
      console.error('[ë¡ê·¸ì¸] ìì¸ ë°ì:', error)
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">BandAuto íë§¤ê´ë¦¬ì</p>
        </div>
