...
export default function TermsPolicyListPage() {
  const router = useRouter()
  const toast = useToast()
  const [items, setItems] = useState<TermsPolicy[]>([])
  ...
  const loadItems = async () => {
    try {
      ...
      const response = await fetch(`/api/policy/terms?${params}`)
  ...
  const confirmDeleteSelected = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(`${successCount}개의 이용약관이 삭제되었습니다.`)
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* 헤더 */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">이용약관 관리</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          {/* 추가 카드 */}
          <button
            onClick={() => router.push('/shop/policy/terms/new')}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500">이용약관</p>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
          ...
          {isLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {items.length === 0 ? (
                  <TableEmpty message="등록된 이용약관이 없습니다." />
                ) : (
                  items.map((item) => (
                    <TableRow
                      key={item.id}
      ...
      <ConfirmModal
        ...
        title="이용약관 삭제"
        message={`선택한 ${selectedIds.length}개의 이용약관을 삭제하시겠습니까?`}
        confirmText="삭제"
        variant="danger"
