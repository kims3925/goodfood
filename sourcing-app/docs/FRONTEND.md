...
### `/sourcing/cleanup` (상품 삭제 관리, 2026-06-10 신설)

- 사이드바 "상품 > 삭제 관리". content-expiry 자동 cron(2026-05-04 비활성)의 수동 대체 경로.
- 조건: 기준일(7/14/30일 퀵버튼)·대상(소매밴드/쇼핑몰)·카테고리 칩·상시상품(COM) 포함 토글(기본 보호)·
  도매 원본 소실(모니터링 감지) 포함·소매밴드 채널 필터.
- 미리보기 → 상품별 체크박스로 **남길 것/삭제할 것 선택**(기본 전체 선택, 해제 = 남김) → 선택분만 실행.
- "소매밴드 실제 게시글도 삭제" 체크 시 Playwright 실삭제(1회 30건 상한), 미체크 시 DB 발행기록만(가역).

### `/sourcing/publish/digest` 변경 (2026-06-10)

- 우측에 "🖼️ 상단 상품목록 포스터 + 상품명 나열" 토글(기본 ON) + 포스터 안내문구 입력.
- ON 이면 종합 게시글 본문 맨 위에 `🧡 상품1 / 상품2 / ... 🧡` 나열 + `👇 오늘의 상품 👇` ✅체크리스트,
  갤러리 맨 앞에 노란 포스터 PNG 1장 (수산천국 접수글 스타일). 댓글 주문링크는 기존 그대로.
