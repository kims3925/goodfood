...
| 항목 | 보안 조치 |
|-----|---------|
| 서비스 계정 JSON | DB 저장 (사용자별 격리) |
| 접근 제어 | 본인 설정만 조회/수정 가능 |
| 로깅 | 서비스 계정 키 내용 로그 금지 |
| 삭제 | 설정 삭제 시 인증 정보 완전 제거 |

**주의사항:**
- 서비스 계정 JSON에는 private_key가 포함되어 있어 민감 정보로 분류
- 프론트엔드로 서비스 계정 키 전송 금지
- API 응답에서 serviceAccountEmail만 노출 (키 내용 제외)
